...
namespace map_render {
    ...
    std::vector<svg::Polyline> MapRender::GetRouteLines(const std::map<std::string_view, const transport_catalogue::Bus*>& buses, const SphereProjector& sp) const {
    ...
    svg::Document MapRender::GetSVG(const std::map<std::string_view, const transport_catalogue::Bus*>& buses) const {
        svg::Document result;
        ...
        for (const auto& text : GetBusMapName(buses, sp)) result.Add(text);
        for (const auto& circle : GetStopsCircle(all_stops, sp)) result.Add(circle);
        for (const auto& text : GetStopsMapName(all_stops, sp)) result.Add(text);

        return result;
    }
    std::vector<svg::Text> MapRender::GetBusMapName(const std::map<std::string_view, const transport_catalogue::Bus*>& buses, const SphereProjector& sp) const {
    ...
    std::vector<svg::Circle> MapRender::GetStopsCircle(const std::map<std::string_view, const transport_catalogue::Stop*>& stops, const SphereProjector& sp) const {
    ...
    std::vector<svg::Text> MapRender::GetStopsMapName(const std::map<std::string_view, const transport_catalogue::Stop*>& stops, const SphereProjector& sp) const {
        std::vector<svg::Text> result;
        svg::Text text;
        svg::Text underlayer;
        for (const auto& [stop_name, stop] : stops) {
